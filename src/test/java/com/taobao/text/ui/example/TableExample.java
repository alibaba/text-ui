...
public class TableExample {
    ...
    public void test1() {
        ...
        // 还有内部的分隔线，默认内部没有分隔线
        TableElement tableElement = new TableElement(1, 1, 1, 97).border(BorderStyle.DASHED).separator(BorderStyle.DASHED);
       ...
       // tableElement.row(true, fields);
        tableElement.row(true,  label("name", 5, true),  label("age", 3),  label("type", 4), label("memo", 4));
        ...
        // 设置第二列字体加粗，加下划线
        for (int i = 0; i < 10; ++i) {
            tableElement.add(row().add(label("student" + i, 0).style(Composite.style(Color.blue).bg(Color.red)))
                    .add(label("" + i).style(Decoration.bold.underline()))
                    .add(label("1"))
                    .add(label("AAAAAAAAACCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC"
                      + "cABBBBBBBBBBBBBBBBAAAAAAAAAAAAAAA AAAAAAAAAAAAAAAAAAAAAAAAAAAA")));
        }

         // 默认输出宽度是80
        final int tableLengths[] = new int[] {40, 80, 100};
        for (final int width : tableLengths) {
          System.err.println(RenderUtil.render(tableElement, width));
        }
    }
}
